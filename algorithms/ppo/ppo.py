...
def trainer(model, exp_buf, sync_evs, ret_queue,args):
    ...
    for epoch in range(args.epochs):
        ...
        if len(rollout_ret) >0:
            return_t = torch.Tensor(rollout_ret).cuda()
            mean_t = torch.mean(return_t)
            mean_t = distributed_avg(mean_t)
            print(f"Epoch [{epoch}] return, max:({max(rollout_ret)}) min:({min(rollout_ret)}), avg:({mean_t.item()})")
        ...
        if args.rank == 0:
            ...
            if len(rollout_ret) >0:
              writer.add_scalar("avg_ret",mean_t,global_steps)
